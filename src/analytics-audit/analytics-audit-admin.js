(function(global) {
  ...
  function AnalyticsAuditAdmin(options) {
    this.options = options || {};
    this.rootSelector = this.options.root || "#analytics-audit-admin-root";
  ...
  AnalyticsAuditAdmin.prototype.init = function() {
  ...
  AnalyticsAuditAdmin.prototype.renderShell = function() {
    ...
    global.jQuery("<h1></h1>").text("Auditoria analytics").appendTo(title);
    global.jQuery("<p></p>").text("Consulta administrativa das consultas BI gravadas no banco separado de auditoria.").appendTo(title);
    ...
    this.datasetInput = this.createSelectField(toolbar, "Dataset");
    ...
    this.gridCard = this.createCard(this.leftColumn, "Consultas");
    ...
    this.detailElement.text("Selecione uma consulta.");
  };

  AnalyticsAuditAdmin.prototype.loadBootstrap = function() {
    this.setStatus("Carregando");
    return this.request("GET", "/api/admin/analytics-audit/bootstrap").then(function(payload) {
      ...
      this.showError(error, "Nao foi possivel carregar a auditoria analytics.");
      throw error;
    }.bind(this));
  };

  AnalyticsAuditAdmin.prototype.loadEntries = function() {
    return this.request("GET", "/api/admin/analytics-audit/entries", this.filters).then(function(payload) {
      ...
      this.showError(error, "Nao foi possivel carregar as consultas auditadas.");
      throw error;
    }.bind(this));
  };

  AnalyticsAuditAdmin.prototype.renderSummary = function() {
    this.summaryBody.find(".manual-summary").remove();
    this.summaryBody.find(".manual-meta").remove();
    if (this.bootstrapData.enabled === false) {
      global.jQuery("<p class=\"manual-summary\"></p>").text("A auditoria analytics esta desabilitada neste ambiente.").appendTo(this.summaryBody);
      return;
    }
    const summary = this.summaryData || this.bootstrapData.summary || {};
    global.jQuery("<p class=\"manual-summary\"></p>").text("Base separada pronta para consulta administrativa das trilhas de BI.").appendTo(this.summaryBody);
    ...
    Object.keys(bySource).slice(0, 4).forEach(function(key) {
      this.appendBadge(badges, key + ": " + String(bySource[key]));
    }, this);
  };

  AnalyticsAuditAdmin.prototype.renderFilters = function() {
    ...
    this.bindDropDown(this.datasetInput, this.bootstrapData.filterOptions && this.bootstrapData.filterOptions.datasetIds || []);
  ...
  AnalyticsAuditAdmin.prototype.renderGrid = function() {
    if (this.gridWidget) {
  ...
  AnalyticsAuditAdmin.prototype.renderDetail = function() {
    this.detailElement.empty();
    const entry = this.currentEntry || this.entries[0] || null;
    if (!entry) {
      this.detailElement.text(this.bootstrapData.enabled === false ? "Auditoria desabilitada." : "Nenhuma consulta encontrada.");
      return;
    }
    ...
    this.appendBadge(badges, "Dataset " + String(entry.datasetId || "-"));
    this.appendBadge(badges, "Origem " + String(entry.resultSource || "-"));
    this.appendBadge(badges, "Execucao " + String(entry.executionMode || "-"));
    this.appendBadge(badges, "Tenant " + String(entry.tenantId || "-"));
    const dl = global.jQuery("<dl class=\"program-builder-technical-properties\"></dl>").appendTo(this.detailElement);
    this.appendDefinition(dl, "Consultado em", entry.consultedAt || "");
    this.appendDefinition(dl, "Usuario", entry.userId || "");
    this.appendDefinition(dl, "Sessao", entry.sessionId || "");
    this.appendDefinition(dl, "View", entry.viewId || "");
  ...
  AnalyticsAuditAdmin.prototype.handleApplyFilters = function() {
    this.filters = {
      ...
      datasetId: this.readDropDownValue(this.datasetInput),
  ...
  AnalyticsAuditAdmin.prototype.handleResetFilters = function() {
    this.filters = {
      tenantId: "",
      userId: "",
      screenId: "",
  ...
  AnalyticsAuditAdmin.prototype.handleExportJson = function() {
    const content = JSON.stringify(this.entries, null, 2);
    this.downloadFile("analytics-audit.json", "application/json", content);
  };

  AnalyticsAuditAdmin.prototype.createCard = function(container, title) {
  ...
  AnalyticsAuditAdmin.prototype.createButton = function(container, text, icon, handler) {
  ...
  AnalyticsAuditAdmin.prototype.createSelectField = function(container, label) {
  ...
  AnalyticsAuditAdmin.prototype.createDateField = function(container, label) {
  ...
  AnalyticsAuditAdmin.prototype.createNumberField = function(container, label, value) {
  ...
  AnalyticsAuditAdmin.prototype.bindDropDown = function(input, items) {
  ...
  AnalyticsAuditAdmin.prototype.readDropDownValue = function(input) {
    const widget = input.data("kendoDropDownList");
    return widget ? String(widget.value() || "") : "";
  };

  AnalyticsAuditAdmin.prototype.appendBadge = function(container, text) {
    return global.jQuery("<span class=\"manual-badge\"></span>").text(text || "").appendTo(container);
  };

  AnalyticsAuditAdmin.prototype.appendDefinition = function(container, title, value) {
  ...
  AnalyticsAuditAdmin.prototype.setStatus = function(text) {
    this.statusBadge.text(text || "");
  };

  AnalyticsAuditAdmin.prototype.showError = function(error, fallback) {
  ...
  AnalyticsAuditAdmin.prototype.request = function(method, url, data) {
    return this.httpClient.request({ method: method, url: url, data: data || {} });
  };

  AnalyticsAuditAdmin.prototype.downloadFile = function(fileName, mimeType, content) {
  ...
  global.AnalyticsAuditAdmin = AnalyticsAuditAdmin;
})(window);
