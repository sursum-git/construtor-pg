...
- existe agora uma terceira trilha interna para documentos de alto rigor:
...
- Existe camada inicial LGPD para pedidos do titular: `production/privacy-request.html`, endpoints publicos `/api/public/privacy/requests/start|confirm|{protocol}`, tabelas `privacy_subject_request`, `privacy_subject_request_verification`, `privacy_subject_request_evidence` e `privacy_retention_policy`, e telas administrativas `admin.lgpd-solicitacoes`, `admin.lgpd-evidencias` e `admin.lgpd-retencao`. Pedido publico exige validacao por e-mail antes de virar pendencia; pedido manual por canal externo usa a tela administrativa e gera o mesmo alerta prioritario, evento `privacy.subject_request.created` e log operacional.
...
- Existe impersonacao administrativa auditavel para suporte: `POST /api/auth/impersonate/start`, `POST /api/auth/impersonate/stop` e acao em `admin.usuarios`. Exige `admin.impersonate`, justificativa obrigatoria, bloqueia usuario inativo, nao gera `rememberToken`, expira em 60 minutos e registra administrador original em `runtime_transaction.requestContext` e `runtime_transaction_log.metadata`.
...
- Banco limpo de validacao: `construtor_pg_validate_eventbus`, criado em PostgreSQL local na porta `5433`, passou por `doctrine:migrations:migrate`, `app:seed-runtime-metadata` e `doctrine:schema:validate` completo com sucesso apos `Version20260527190801`. No banco local principal, o `schema:validate` completo ainda pode acusar apenas tabelas dinamicas/de teste criadas pelo construtor (`pedido_builder_v2`, `produto_builder`, `t3300` etc.); nesse caso, a diferenca esperada aparece como `DROP TABLE` dessas tabelas no `doctrine:schema:update --dump-sql`.
